...
def slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    for t in range(1,3,1):
        ...
        Xi = matrix([[0] for row in range(N+num_landmarks)])
         
        # add initial constraint 
        tmp = [[0 for row in range(N + num_landmarks)] for col in range(N + num_landmarks)]
        tmp[0][0] = 1
        omega += matrix(tmp)
        
        tmp = [[0] for row in range(N+num_landmarks)]
        tmp[0][0] = 50  
        Xi += matrix(tmp)
        ...
        for i in range(N-1):
            #sense
            # data[i][0] format
            #[[0, -19.493332471235703, 7.359826712698734], [3, 18.118507809788873, -4.70615298297017]]
            for landmark in data[i][0]:
                landmark_id = landmark[0]
                
                tmp = [[0 for row in range(N + num_landmarks)] for col in range(N + num_landmarks)]
                tmp[landmark_id + N ][landmark_id + N ] = 1.0  
                tmp[landmark_id + N ][i] = (-1.0) 
                tmp[i][i] = 1.0 
                tmp[i][landmark_id + N ] = (-1.0) 
                omega += matrix(tmp)
                
                tmp = [[0] for row in range(N+num_landmarks)]
                tmp[i][0] = - landmark[t] 
                tmp[landmark_id + N][0] = landmark[t] 
                Xi += matrix(tmp)
            ...
            tmp = [[0] for row in range(N+num_landmarks)]
            tmp[i][0] = - data[i][1][t-1] 
            tmp[i+1][0] = data[i][1][t-1] 
            
            Xi += matrix(tmp)

        mu = omega.inverse() * Xi
        mu_lst.append(mu.value)

    mu = []
    for i in range(len(mu_lst[0])):
        mu.append(mu_lst[0][i])
        mu.append(mu_lst[1][i])
    mu = matrix(mu)
    return mu 
