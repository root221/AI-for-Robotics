...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        if(abs(beta) < 0.001):
            ...
            radius = self.length / tan(steering_angle)
            x = self.x + (- radius * sin(self.orientation)) + radius * sin(beta + self.orientation)
            y = self.y + radius * cos(self.orientation) - radius * cos(beta + self.orientation) 
            orientation = self.orientation + beta
